"""BLS sync tests: directory traversal, discovery, sync, archiving."""
import sys
...
class TestFileSync:
    ...
    def test_sync_file_unchanged(
        ...
        mock_get_etag.return_value = md5
        mock_upload.return_value = True
        result = sync_file(s3_client, 'bucket', 'https://example.com/file.txt', 'file.txt')
        assert result is True
    ...
    def test_sync_file_changed(
        ...
        mock_fetch.return_value = content
        mock_get_etag.return_value = old_md5
        mock_upload.return_value = True
        result = sync_file(s3_client, 'bucket', 'https://example.com/file.txt', 'file.txt')
        assert result is True
        mock_upload.assert_called_once()
...
class TestFullSync:
    ...
    def test_sync_bls_data_empty_archive_prefix_still_archives(
        ...
        """archive_prefix='' must not make bls_files_in_s3 empty (startswith('') is True for all)."""
        s3_client = Mock()
        mock_boto3_client.return_value = s3_client
        discovered_files = {'file1.txt', 'file2.txt'}
        mock_sync_iterative.return_value = discovered_files
